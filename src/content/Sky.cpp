...
void Sky::interpolate()
{
    ...
    for(size_t i = 0; i < m_MasterState.layers.size(); i++)
    {
        ...
        m_MasterState.layers[i].textureAlpha = bx::lerp(s0.layers[i].textureAlpha, s1.layers[i].textureAlpha, t);
        m_MasterState.layers[i].textureScale = bx::lerp(s0.layers[i].textureScale, s1.layers[i].textureScale, t);
    }
...
static void applyAlphaFadeToDomeColorPart(ZenLoad::PackedMesh& mesh)
{
    ...
    for(ZenLoad::WorldVertex& vx : mesh.vertices)
    {
        ...
        if(vertexHeightRatio < fadeout_offset) {
            alpha = bx::lerp(0.0f, 0.8f, Math::sinusSlowEnd(vertexHeightRatio  / fadeout_offset));
        } else {
            alpha = bx::lerp(0.8f, 0.0f, Math::sinusSlowEnd(vertexHeightRatio) - fadeout_offset);
        }
